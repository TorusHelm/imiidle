...
func test_slot_overlays_show_coin_feedback_and_modifier_status() -> void:
	...
	var floating_coin: Sprite2D = null
	var floating_amount_label: Label = null

	for child in floating_feedback_layer.get_children():
		if child is Sprite2D and floating_coin == null:
			floating_coin = child
		elif child is Label and floating_amount_label == null:
	...
	assert_not_null(floating_coin, "Source slot should render a floating coin sprite after plant activation.")
	assert_not_null(floating_amount_label, "Source slot should render the gained coin amount next to the floating coin.")
	if floating_amount_label != null:
		assert_eq(floating_amount_label.text, "+1.7", "Floating coin feedback should show the reward generated by the plant.")
	if floating_amount_label != null and floating_coin != null:
		assert_lt(floating_amount_label.position.x, floating_coin.position.x, "Floating amount label should appear to the left of the coin icon.")
